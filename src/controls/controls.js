...
import { claimFee, getBalance, getklonSum} from '../web3';
...
export async function clickManager(e) {
    ...
    if(FAQ){
    ...
    }else if (isInSquare(mousePos, 0, GUICatalog.pixelMinted.img.width, 0, GUICatalog.pixelMinted.img.height, 'pixelMinted', 'GUICatalog')) {
        let pix = await getklonSum();
        alert("PixelMinted "+pix );       

    }else if (isInSquare(mousePos, 0, GUICatalog.faqButtonOpen.img.width, 0, GUICatalog.faqButtonOpen.img.height, 'faqButtonOpen', 'GUICatalog')){
        displayFAQ('FAQ');
        ...
        window.open('https://twitter.com/ftm_building', '_blank');
    } else if (isInCircle(mousePos, 72, 72, 13, 'planLogos', 'imageCatalog')) {
        window.open('https://cryptomage.notion.site/Building-ftm-a31ebe1598a0434f9618e132fc248578', '_blank');
